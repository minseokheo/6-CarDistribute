...
vid = cv2.VideoCapture('C:/Users/HOME/Desktop/project/video/video.mp4') # 원본 영상 경로, 원본 영상 불러오기
f = open('C:/Users/HOME/Desktop/project/video/new_label/vid_convert.txt', 'r') # Detection하고 포맷 바꾼 파일 경로

start = time.time()
os.environ["CUDA_VISIBLE_DEVICES"] = '0,1' 
motdata = 'C:/Users/HOME/Desktop/project/video/crop_video' # 첨부한 v3폴더의 경로(동영상을 1fps마다 자른 것)
print(motdata)
...
with open(f'C:/Users/HOME/Desktop/project/video/video.json', 'w') as make_file: # json 파일 만드는 코드
  json.dump(file_data, make_file, ensure_ascii=False)
##########

matplotlib.use('Agg') 
jsonpath = f'C:/Users/HOME/Desktop/project/video/video.json' # 바로 위에서 만든 json 파일의 경로
...
save_path = f'C:/Users/HOME/Desktop/project/video/trackingvideo/' # 영상이 저장될 경로 지정
########## Tracking 결과 영상이 저장될 경로로 이동
os.chdir('C:/Users/HOME/Desktop/project/video/trackingvideo/')
os.system(f'del resultvid')
...
out = cv2.VideoWriter(f'C:/Users/HOME/Desktop/project/video/trackingvideo/resultvid.avi', codec, vid_fps, (vid_width, vid_height))
##########

########## SORT 알고리즘 동작 코드
from collections import deque
pts = [deque(maxlen = 10155) for _ in range(50000)] # 차들이 이동한 흔적 좌표를 저장
sort = 'C:/Users/HOME/Desktop/sort/sort-master/' # SORT 알고리즘 설치한 곳 경로 지정
...
with tf.device("/device:GPU:0"): # GPU를 사용하여 tracking 수행
    ...
    #####
    f2 = open(f"C:/Users/HOME/Desktop/project/video/trackingvideo/Tracking_coordinate.txt", "a") # Tracking 결과 좌표를 저장하기 위해 a로 오픈
    for key in natsort.natsorted(odata.keys()): # odata에 있는 key값들을 불러옴
      ...
      for info in det_result:
        ...
        ##### 수정한부분(7/24) #####
      if len(arrlist) == 0:
        tmp = [str(fr), str(0), str(0), str(0), str(0), str(0), str(10)]
        f2.write(' '.join(tmp) + '\n')
        continue
